...
/**
 * Predicts referrals to external centers until the cutoff date,
 * broken down by surgical service and by reason.
 */
async function predictReferrals(cutoffDate) {
  const now = new Date();
  ...
  const oldest = await ReferralRecord.findOne().sort({ referralDate: 1 }).lean();
  const histStart = oldest ? new Date(oldest.referralDate) : new Date(now.getFullYear() - 1, now.getMonth(), now.getDate());
  const histMonths = Math.max(1, (now - histStart) / (1000 * 60 * 60 * 24 * 30));

  // Aggregate by service
  const byServiceAgg = await ReferralRecord.aggregate([
  ...
  // Aggregate by reason
  const byReasonAgg = await ReferralRecord.aggregate([
